...
##
addAICc=function (object, scope, scale = 0, test = c("none", "Chisq"),
k = 2, sorted = FALSE, trace = FALSE, ...)
{
...
aod <- dropAICc(fit, scope$drop, scale = scale, trace = max(0,
trace - 1), k = k, ...)
rn <- row.names(aod)
...
aodf <- addAICc(fit, scope$add, scale = scale,
...
for (j in 1:100){
  ...
  fitnull = lm(y~1, data = dataset)
  fitfull = lm(y~x1+x2+x3+x4+x5+x6+x7, data = dataset)
  
  #use AIC
  AIC.step= step(fitnull,direction = 
  "both",scope=list(upper=fitfull,lower=fitnull))
  count[1, AIC.step$rank-1]=count[1, AIC.step$rank-1]+1
  
  #use BIC
  BIC.step = step(fitnull, direction = "both", k=log(n), 
  scope=list(upper=fitfull,lower=fitnull))
  count[2, BIC.step$rank-1]=count[2, BIC.step$rank-1]+1
    
  #use AICC
  AICC.step = stepAICc(fitnull, direction = "both", scope=list(upper=fitfull,lower=fitnull))
  count[3, AICC.step$rank-1]=count[3, AICC.step$rank-1]+1
}
